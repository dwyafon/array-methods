...
test('returns a string representing the passed array', () => {
  expect(functions.arrToString([1, 2, 3, '4', '5', '6', true, false])).toEqual(
...
test('mutates the array by adding the passed values to the passed array and return the mutated array', () => {
  expect(functions.arrUnshift([0, 1, 2, 3], -3, -2, -1)).toEqual([
    -3,
    -2,
    -1,
    0,
...
//arrValues
test('returns an array interator object with values for value and done', () => {
  expect(functions.arrValues([1, 2, 3, 4, 5])).toEqual({done: false, value: 1})
})
